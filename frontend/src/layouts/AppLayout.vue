...
const nav = [
  { to: '/dashboard', label: '学习驾驶舱', icon: HomeFilled },
  { to: '/plan', label: '八周作战图', icon: Calendar },
...
const copy: Record<string, [string, string]> = {
  '/dashboard': ['学习驾驶舱', '今天要做什么、哪些错误该复习、哪里正在反复薄弱。'],
  '/plan': ['八周作战图', '从原 HTML 迁移来的主线计划，现在可以导入、拆分和打卡。'],
  '/goals': ['学习目标', '从模板生成，也可以完全自定义阶段、任务和截止时间。'],
  '/tasks': ['学习任务', '把目标拆成今天能完成的一小步。'],
  '/errors': ['错误记录', '用 Markdown、代码块、标签和间隔复习把坑变成资产。'],
  '/reviews': ['今日复习', '按掌握程度自动推迟下次复习时间。'],
  '/statistics': ['数据统计', '用任务趋势、错误类型和标签分布定位薄弱点。'],
  '/notes': ['学习笔记', '导入 Markdown、PDF、Word，沉淀成可导出的复盘材料。'],
  '/settings': ['个人设置', '账号、主题和学习工作台偏好。']
}

const title = computed(() => copy[route.path]?.[0] || 'LearnTrace')
const subtitle = computed(() => copy[route.path]?.[1] || '学习闭环系统')
